...
function webSocketOnMessage(event){
    ...
    // in case of new peer
    if(action == 'new-peer'){
        console.log('New peer: ', Userscount);
    ...
    .then(stream => {
        ...
        btnToggleAudio.onclick = function(){
            ...
            if(audioTracks[0].enabled){
                btnToggleAudio.classList.add("fa microphone");
                btnToggleAudio.classList.remove("fa microphone-slash");
                return;
            }
            
            btnToggleAudio.classList.add("fa microphone-slash");
        };

        btnToggleVideo.onclick = function(){
            videoTracks[0].enabled = !videoTracks[0].enabled;
            if(videoTracks[0].enabled){
                btnToggleVideo.classList.add("fa video");
                btnToggleVideo.classList.remove("fa video-slash");
                return;
            }

            btnToggleVideo.classList.add("fa video-slash");
        };
    })
    .then(e => {
